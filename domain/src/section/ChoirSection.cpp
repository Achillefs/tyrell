...
ChoirSection::ChoirSection(int sample_rate) : lower_bank_{sample_rate}, upper_bank_{sample_rate} {
}
...
static void process_zone(ChoirFilterBank& bank, const float* in_8, const float* in_4, bool sw_8,
                         bool sw_4, int sw_8_idx, int sw_4_idx, float* left, float* right,
                         std::size_t frames, std::vector<float>& scratch) {
  if (!sw_8 && !sw_4) return;
...
void ChoirSection::process(const float* lower_8, const float* lower_4, const float* upper_8,
                           const float* upper_4, float* left, float* right, std::size_t frames) {
  std::fill_n(left, frames, 0.f);
  std::fill_n(right, frames, 0.f);

  // Lower zone: LowerMale8 (sw=0) uses 8′, LowerMale4 (sw=1) uses 4′.
  process_zone(lower_bank_, lower_8, lower_4, switches_[0], switches_[1], 0, 1, left, right, frames,
               mix_scratch_);

  // Upper zone: UpperMale8 (sw=2) uses 8′, UpperFemale4 (sw=3) uses 4′.
  process_zone(upper_bank_, upper_8, upper_4, switches_[2], switches_[3], 2, 3, left, right, frames,
               mix_scratch_);
}
