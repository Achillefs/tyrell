...
TopOctaveDivider::TopOctaveDivider(Hertz master_clock, std::array<int, 12> divider_ratios,
                                   int sample_rate)
    : master_clock_{master_clock}, divider_ratios_{divider_ratios}, sample_rate_{sample_rate} {
}
...
void TopOctaveDivider::render_pitch_class(int pitch_class, float* out, std::size_t frames) {
  ...
  for (std::size_t i = 0; i < frames; ++i) {
    ...
    const double p_raw =
        std::fmod(static_cast<double>(n) * freq * inv_sr + phase_offsets_[pitch_class], 1.0);
